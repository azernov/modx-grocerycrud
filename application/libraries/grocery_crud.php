...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    protected $modx_path                = '/core/components/opencolour';
	...
	{			
		if(!file_exists(dirname(dirname(dirname(__FILE__))).'/'.$this->theme_path.$this->theme.'/views/list_template.php'))
		{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		{
			$data->list[$num_row]->edit_url = $data->edit_url.'&id='.$row->{$data->primary_key};
			$data->list[$num_row]->delete_url = $data->delete_url.'&id='.$row->{$data->primary_key};
		}
	...
	{
		$this->css_files[sha1($css_file)] = $this->modx_path.'/'.$css_file;
	}

	public function set_js($js_file)
	{
		$this->js_files[sha1($js_file)] = $this->modx_path.'/'.$js_file;
	}
	...
	protected function setThemeBasics()
	{
		...
		include(dirname(dirname(dirname(__FILE__))).'/'.$this->theme_path.$this->theme.'/config.php');
	...
	protected function _theme_view($view, $vars = array(), $return = FALSE)
	{
		...
		if (file_exists(dirname(dirname(dirname(__FILE__))).'/'.$view_file.$file))
		{
			$path = dirname(dirname(dirname(__FILE__))).'/'.$view_file.$file;
			$file_exists = TRUE;
		}
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function state_url($url = '')
	{
        ...
        {
            if($parameter != 'action' && $parameter != 'id' &&  $parameter != 'id1')
            {
        ...
        $retString = 'http://'.$_SERVER['HTTP_HOST'].$urlArray['path'].'?'.$params;

        //$retString = preg_replace('/^([^?]+)(\?.*?)?(#.*)?$/', '$1$3', $_SERVER['REQUEST_URI']).'?'.$params;

        $segments = explode('/',$url);

        if(isset($segments[0]))
        {
            $retString .= '&action='.$segments[0];
        }
        if(isset($segments[1]))
        {
            $retString .= '&id='.$segments[1];
        }
        if(isset($segments[2]))
        {
            $retString .= '&id1='.$segments[2];
        }
        /*if($segments[0]=='edit')
        {
            var_dump($retString);
	...
	protected function get_state_info_from_url()
	{
        ...
        $first_parameter = isset($_GET['id']) ? $_GET['id'] : null;
        $second_parameter = isset($_GET['id1']) ? $_GET['id1'] : null;
...
class grocery_CRUD extends grocery_CRUD_States
{
	...
	{
        //$this->default_assets_path = dirname(dirname(dirname(__FILE__))).'/'.$this->default_assets_path;
        //$this->default_config_path = dirname(dirname(dirname(__FILE__))).'/'.$this->default_config_path;
        //$this->default_language_path = dirname(dirname(dirname(__FILE__))).'/'.$this->default_language_path;
	}	
	...
	protected function _load_language()
	{
		...
		include(dirname(dirname(dirname(__FILE__))).'/'.$this->default_language_path.'/'.$this->language.'.php');
